          ...
          @if (isAuthenticated()) {
            <div class="card review-form">
              ...
              <div class="form-group">
                ...
                <textarea rows="3" [ngModel]="myComment()" (ngModelChange)="myComment.set($event)"></textarea>
              </div>
...
export class DetailComponent {
  ...
  isLoadingScreenings = signal(false);
  ...
  isInCart = computed(() => {
    ...
    if (!id) return false;
    return this.bookingService.isInCart(id);
  });
  ...
  private loadScreenings(): void {
    ...
    this.screeningService.getScreenings(id).pipe(map((r: any) => r.data as Screening[])).subscribe({
      next: (screenings) => {
        this.screenings.set(screenings || []);
        this.isLoadingScreenings.set(false);
      },
      error: (err) => {
        this.screeningsError.set(err?.error?.message || 'Failed to load showtimes');
        this.isLoadingScreenings.set(false);
      },
    });
  }
  ...
  submitReview(): void {
    ...
    this.reviewService.createReview(id, {
      rating,
